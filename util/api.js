  ...
  let posts = await contentfulClient
    ...
    .then((entries) => {
      ...
      const itemsWithThisSlug = items.filter((entry) => {
        ...
        const slugVal = _.get(entry, "fields.slug");
        console.log("given-slug:".slugVal);
        if (slugVal === slug) {
          return fields;
        }
      });
...
// Basic Page Code Started
export const getPages = async (slug, preview) => {
  const options = getOptions(preview);

  const contentfulClient = contentful.createClient(options);

  let bpage = await contentfulClient
    .getEntries({
      content_type: "page",
    })
    .then((entries) => {
      ...
      //   all others -> morePosts - Other pages
      const itemsWithoutThisSlug = items.filter((entry) => {
        const slugVal = _.get(entry, "fields.slug");
        if (slugVal != slug) {
          return entry;
...
export const getProducts = async (slug, preview) => {
  const options = getOptions(preview);
